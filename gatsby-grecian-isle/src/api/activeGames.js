...
class serverMethodsActiveGames {
  deleteGame(gameId) {
    ActiveGames.remove(gameId);
  };

  createNewGame(username, selectedGame, localGame) {
...
class serverMethodsGame {
  ...
  removePlayer(id, userId, creatingPlayer, joiningPlayer) {
    if (userId === 1) {
      ActiveGames.update(id, {
        $set: {
  ...
  handleSelectionInPlacementPhase(id, newData) {
    ActiveGames.update(id, {
      $inc: {
        workerBeingPlaced: 1,
      },
  ...
  handleSelectionInSelectPhase(id, newData) {
    ActiveGames.update(id, {
      $set: {
        turnPhase: newData.turnPhase,
  ...
  handleSelectionInMovePhase(id, newData) {
    ActiveGames.update(id, {
      $set: {
  ...
  handleSelectionInBuildPhase(id, newData) {
    ActiveGames.update(id, {
      $set: {
        turnPhase: newData.turnPhase,
        activePlayer: newData.activePlayer,
  ...
  resolveRequestToJoin(id, acceptRequest, joiningPlayer) {
    if (acceptRequest) {
      ActiveGames.update(id, {
        $set: {
          pendingRequest: null,
...
class Methods {
  constructor() {
    this.activeGames = new serverMethodsActiveGames(),
    this.game = new serverMethodsGame()
  }
}
